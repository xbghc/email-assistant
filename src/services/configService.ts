...
class ConfigService {
      ...
      this.config = {
        email: {
          ...
          imap: {
            ...
            user: envVars.IMAP_USER || '',
            pass: envVars.IMAP_PASS || '',
  ...
  validateConfig(config: SystemConfig): { isValid: boolean; errors: string[] } {
    ...
    if (!config.email.smtp.user) errors.push('SMTP user is required');
    if (!config.email.user.email) errors.push('User email is required');
  ...
  private generateEnvContent(config: SystemConfig): string {
    const lines = [
      ...
      `SMTP_PASS=${config.email.smtp.pass}`,
      '',
      `IMAP_HOST=${config.email.imap.host}`,
      `IMAP_PORT=${config.email.imap.port}`,
      `IMAP_USER=${config.email.imap.user}`,
      `IMAP_PASS=${config.email.imap.pass}`,
      `IMAP_TLS=${config.email.imap.tls}`,
      '',
      `USER_EMAIL=${config.email.user.email}`,
      `USER_NAME=${config.email.user.name}`,
      '',
      '# AI Configuration',
