...
import { LogQuery } from '../services/logReaderService';
...
router.get('/api/system/status', authenticate, async (req, res) => {
  try {
    const SystemHealthService = (await import('../services/systemHealthService')).default;
    ...
    try {
      ...
      if (!type || type === 'suggestions') {
        reports.push({
          ...
          content: `个性化建议：\n- 邮件发送效率良好\n- 用户活跃度：${Math.round(users.filter((u) => u.isActive).length / users.length * 100)}%\n- 建议定期检查系统性能`
        });
      }
...
router.get('/api/logs', authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    const LogReaderService = (await import('../services/logReaderService')).default;
